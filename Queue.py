...
# (this function was) Made by Ethan O'Connor
# Spire ID: 34445111
    def createList(self):                          
        return str(self).rstrip(" ").split(sep=" ")
        """A simple way to not edit the queue while returning an easily-usable
        version of it. Completely stops the need for copy.copy(), I figured
        it was a reasonable choice. Uses Queue's built-in __str__ (above)

        rstrip() removes the ending space, split() makes it a list
        Format example:
        ['2','3','/','pi','swap','cos','3','**',...]                       """
